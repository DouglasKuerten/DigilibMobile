...
import { IconButton, CloseIcon, Alert, VStack, HStack, Text } from 'native-base'

export function ToastAlert({ id, status, title, description, varToast, ...rest }) {
    let variant = 'left-accent';
    let isClosable = true;
    return (
        <Alert maxWidth="90%" alignSelf="center" flexDirection="row" status={status ? status : "info"} variant={variant} {...rest}>
            <VStack space={1} flexShrink={1} w="100%">
                <HStack flexShrink={1} alignItems="center" justifyContent="space-between">
                    ...
                    }} onPress={() => varToast.close(id)} /> : null}
                </HStack>
